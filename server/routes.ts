...
export async function registerRoutes(
  ...
  app.post("/api/signals/generate", async (req, res) => {
    try {
      ...
      // These are deterministic guardrails that must be met before AI analysis
      const tvDirection = tvAnalysis.recommendation; // 'UP' or 'DOWN' or null
      const hasValidDirection = tvDirection === 'UP' || tvDirection === 'DOWN';
      const hasStrongSignal = Math.abs(recommendAll) >= 0.25;
      const hasMinimumTrend = realAdx >= 18;
      
      // HARD REQUIREMENT: TradingView must have a direction signal
      if (!hasValidDirection) {
        return res.status(200).json({
          noEntry: true,
          analysis: `Немає сигналу | TV: NEUTRAL (${recommendAll.toFixed(2)}) | RSI: ${realRsi.toFixed(0)} | ADX: ${realAdx.toFixed(0)}`,
          pair,
        });
      }
      
      // HARD REQUIREMENT: Signal strength must be >= 0.25
      if (!hasStrongSignal) {
        return res.status(200).json({
          noEntry: true,
          analysis: `Слабкий сигнал | TV: ${recommendAll.toFixed(2)} (потрібно 0.25+) | RSI: ${realRsi.toFixed(0)} | ADX: ${realAdx.toFixed(0)}`,
          pair,
        });
      }
      
      // HARD REQUIREMENT: ADX must show trend presence
      if (!hasMinimumTrend) {
        return res.status(200).json({
          noEntry: true,
          analysis: `Немає тренду | ADX: ${realAdx.toFixed(0)} (потрібно 18+) | TV: ${recommendAll.toFixed(2)} | RSI: ${realRsi.toFixed(0)}`,
          pair,
        });
      }
      
      // ========== PROFESSIONAL AI TRADER ANALYSIS ==========
      // AI can only CONFIRM or REJECT - direction is set by TradingView
      const rsiState = realRsi < 30 ? 'ПЕРЕПРОДАНІСТЬ' : realRsi > 70 ? 'ПЕРЕКУПЛЕНІСТЬ' : 'НЕЙТРАЛЬНО';
...
{"trade":true/false,"confidence":85-99,"analysis":"2-3 речення українською чому входити або чекати"}`;
      ...
      // If AI didn't give a valid response, use fallback with TradingView direction
      if (!aiDecision || typeof aiDecision.trade !== 'boolean') {
        // Fallback: AI failed, but we have TradingView confirmation - use it
        aiDecision = {
          trade: true,
          confidence: Math.min(92, 85 + Math.round(Math.abs(recommendAll) * 15)),
          analysis: `TradingView ${tvAnalysis.signal} (Recommend.All=${recommendAll.toFixed(2)}). RSI=${realRsi.toFixed(0)}, ADX=${realAdx.toFixed(0)}. Індикатори підтверджують напрямок.`
        };
      }
      
      // AI said no trade - respect AI decision
      if (!aiDecision.trade) {
        return res.status(200).json({
          noEntry: true,
          analysis: `${aiDecision.analysis || 'AI рекомендує почекати'} | RSI: ${realRsi.toFixed(0)} | ADX: ${realAdx.toFixed(0)}`,
          pair,
        });
      }
      
      // AI APPROVED - CREATE SIGNAL (direction always from TradingView)
      const sparkline = priceHistory.slice(-6);
      const confidence = Math.max(85, Math.min(99, aiDecision.confidence || 90));
      const direction = tvDirection; // Direction is ALWAYS from TradingView
      
      const analysisDetails = [
        `${confidence}% | AI TRADER АНАЛІЗ`,
        `TV: ${tvAnalysis.signal} | RSI: ${realRsi.toFixed(0)} | ADX: ${realAdx.toFixed(0)} | MACD: ${macdState}`,
        `${aiDecision.analysis}`
      ].join('\n');
